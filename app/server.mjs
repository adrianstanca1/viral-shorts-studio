...
app.get('/api/creator-agent/runs',(req,res)=>{const runs=listRuns(DATA).slice(0,30).map(run=>({...run,artifacts:(run.artifacts||[]).map(a=>a.type==='video'&&a.id?({...a,projectStatus:load(a.id)?.status||a.status,progress:load(a.id)?.progress??null,launchReady:load(a.id)?.qa?.launchReady===true,approval:load(a.id)?.publishApproval?.status||null}):a)}));res.json({runs})});
