...
app.post('/api/system-loop/run',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});if(req.body?.confirm!==true)return res.status(409).json({error:'explicit confirm=true is required'});const projects=list(),perf=performanceSummary(DATA,projects),learning=crossPlatformLearning(listPlatformPerformance(DATA),projects),creative=creativeIntelligence(projects,perf,experimentInsights(DATA));refreshCreativeMemory(DATA,projects,perf,experimentInsights(DATA));const campaigns=listCampaigns(DATA).filter(c=>['planning','active'].includes(c.status)),plans=campaigns.map(c=>campaignLoopPlan(c,{projects,performance:perf,publishJobs:listPublishJobs(DATA,{}),creative,learning})),reports=campaigns.filter(c=>(c.projectIds||[]).length).map(c=>saveCampaignLearningReport(DATA,buildCampaignLearningReport(c,{projects,performance:perf,publishJobs:listPublishJobs(DATA,{}),creative,learning}))),runs=listRuns(DATA).map(run=>syncRunWithProjects(DATA,run,load)).map(run=>({...run,handoffs:syncAgentHandoffs(DATA,run)})),decisions=nextBestActions({campaigns,campaignPlans:plans,performance:perf,creative,learning,runs,feedback:decisionFeedbackSummary(DATA)}),cycle=recordLoopCycle(DATA,{actions:decisions.actions,campaignReports:reports.length,runsChecked:runs.length,ownerConfirmed:true});res.json({status:'cycle-complete',cycle,decisions,campaignReports:reports.length,runsChecked:runs.length,postingTimes:postingTimeIntelligence(listPlatformPerformance(DATA),listPublishJobs(DATA,{})),feedback:decisionFeedbackSummary(DATA),policy:{ownerConfirmed:true,autoPublish:false,freeOnly:true}})});
