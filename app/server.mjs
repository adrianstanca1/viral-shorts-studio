...
app.post('/api/projects/:id/publish-jobs',(req,res)=>{const j=load(req.params.id);if(!j)return res.status(404).json({error:'not found'});try{res.status(201).json(createPublishJob(DATA,j,{platform:String(req.body?.platform||'youtube-shorts'),scheduledAt:req.body?.scheduledAt||null}))}catch(e){res.status(409).json({error:String(e.message||e)})}});
