...
import { buildCreatorAnalytics, recordAnalyticsSnapshot, readAnalytics, listExperiments, createExperiment, updateExperiment, experimentInsights } from './creator-analytics.mjs';
...
app.post('/api/performance/youtube/sync',async(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});const env=youtubeAnalyticsCredentialEnv(DATA),status=youtubeAnalyticsStatus(env);if(!status.enabled)return res.status(409).json({status,error:'YouTube Analytics is not enabled'});const jobs=listPublishJobs(DATA,{}).filter(j=>j.platform==='youtube-shorts'&&j.status==='published'&&j.delivery?.result?.videoId).slice(0,25),records=[],errors=[];for(const job of jobs){try{const a=await fetchYouTubeVideoAnalytics({videoId:job.delivery.result.videoId,days:Number(req.body?.days||28),env});records.push(recordPlatformPerformance(DATA,{platform:'youtube',projectId:job.projectId,externalId:a.videoId,periodStart:a.startDate,periodEnd:a.endDate,metrics:a.metrics,source:'youtube-analytics'}))}catch(e){errors.push({projectId:job.projectId,error:String(e.message||e).slice(0,160)})}}res.json({status,checked:jobs.length,imported:records.length,errors,summary:performanceSummary(DATA,list())})});
app.post('/api/performance/tiktok/sync',async(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});const status=tiktokAnalyticsStatus();if(!status.enabled)return res.status(409).json({status,error:status.configured&&!status.scoped?'TikTok video.list scope is not granted':'TikTok analytics is not enabled'});const jobs=listPublishJobs(DATA,{}).filter(j=>j.platform==='tiktok'&&j.status==='published'&&j.delivery?.result?.publishId).slice(0,20),records=[],errors=[];for(const job of jobs){try{const r=job.delivery.result,st=await fetchTikTokPostStatus(r.publishId),ids=r.postId?[r.postId]:(st.publicaly_available_post_id||st.publicly_available_post_id||[]);if(!ids.length)throw Error('TikTok post id is not publicly available yet');const a=await fetchTikTokVideoAnalytics({videoId:ids[0]});records.push(recordPlatformPerformance(DATA,{platform:'tiktok',projectId:job.projectId,externalId:a.videoId,metrics:a.metrics,source:'tiktok-display-api'}))}catch(e){errors.push({projectId:job.projectId,error:String(e.message||e).slice(0,160)})}}res.json({status,checked:jobs.length,imported:records.length,errors,summary:performanceSummary(DATA,list())})});
app.post('/api/performance/instagram/sync',async(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});const status=instagramAnalyticsStatus();if(!status.enabled)return res.status(409).json({status,error:'Instagram analytics is not enabled'});const jobs=listPublishJobs(DATA,{}).filter(j=>j.platform==='instagram-reels'&&j.status==='published'&&j.delivery?.result?.mediaId).slice(0,20),records=[],errors=[];for(const job of jobs){try{const a=await fetchInstagramMediaAnalytics({mediaId:job.delivery.result.mediaId});records.push(recordPlatformPerformance(DATA,{platform:'instagram',projectId:job.projectId,externalId:a.mediaId,metrics:a.metrics,source:'instagram-graph-api'}))}catch(e){errors.push({projectId:job.projectId,error:String(e.message||e).slice(0,160)})}}res.json({status,checked:jobs.length,imported:records.length,errors,summary:performanceSummary(DATA,list())})});
...
function planCreatorGoalWithSignals(input={}){const base=planCreatorGoal(input),perf=performanceSummary(DATA,list()),series=contentSeriesRecommendations(list(),perf);return performanceAwareCreatorPlan(base,{performance:perf,series});}
...
app.post('/api/creator-agent/plan',(req,res)=>{try{res.status(201).json(planCreatorGoalWithSignals(req.body||{}))}catch(e){res.status(400).json({error:String(e.message||e)})}});
