...
import { recordPlatformPerformance, listPlatformPerformance, performanceSummary, performanceActions } from './platform-performance.mjs';
...
app.get('/api/performance',(req,res)=>res.json({records:listPlatformPerformance(DATA,{platform:String(req.query.platform||''),projectId:String(req.query.projectId||'')}),summary:performanceSummary(DATA,list()),actions:performanceActions(DATA,list())}));
...
app.get('/api/experiments/allocation',(req,res)=>{const perf=performanceSummary(DATA,list()),boost=new Map((perf.top||[]).map(x=>[x.projectId,Math.min(20,Math.round(x.score/5))]));const recommendations=experimentAllocation(list(),listExperiments(DATA)).map(x=>({...x,observedPerformanceBoost:boost.get(x.projectId)||0,priority:x.priority+(boost.get(x.projectId)||0)})).sort((a,b)=>b.priority-a.priority);res.json({recommendations,performance:{records:perf.records,projects:perf.projects},policy:{ownerReviewRequired:true,autoCreate:false}})});
...
app.get('/api/publishing/connectors',(req,res)=>res.json({google:googleOAuthStatus(DATA),youtube:youtubePublisherStatus(youtubeCredentialEnv(DATA)),tiktok:tiktokPublisherStatus(),instagram:instagramPublisherStatus(),youtubeAnalytics:youtubeAnalyticsStatus(youtubeAnalyticsCredentialEnv(DATA))}));
