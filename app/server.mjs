...
app.get('/api/publishing/connectors',(req,res)=>res.json({google:googleOAuthStatus(DATA),youtube:youtubePublisherStatus(youtubeCredentialEnv(DATA)),tiktok:tiktokPublisherStatus(),instagram:{id:'instagram',configured:false,enabled:false,externalPostingEnabled:false,mode:'manual-export'},youtubeAnalytics:youtubeAnalyticsStatus(youtubeAnalyticsCredentialEnv(DATA))}));
...
async function executePublishJob(job){
  if(!job)throw new Error('publish job not found');if(!['youtube-shorts','tiktok'].includes(job.platform))throw new Error('platform connector not implemented');
  ...
  publishingExecutions.add(job.id);try{updatePublishJob(DATA,job.id,{status:'uploading',lastError:null});const result=job.platform==='youtube-shorts'?await uploadYouTubeVideo({videoFile:project.render?.file,pkg:job.package,scheduledAt:job.scheduledAt,env:youtubeCredentialEnv(DATA)}):await uploadTikTokVideo({videoFile:project.render?.file,pkg:job.package});return updatePublishJob(DATA,job.id,{status:'published',delivery:{mode:job.platform==='youtube-shorts'?'youtube-api':'tiktok-content-posting-api',externalPostingEnabled:true,result},publishedAt:new Date().toISOString()});}catch(e){updatePublishJob(DATA,job.id,{status:'failed',lastError:String(e.message||e).slice(0,240)});throw e}finally{publishingExecutions.delete(job.id)}
}
