...
import { listChannelGoals, createChannelGoal, updateChannelGoal, channelGoalProgress } from './channel-goals.mjs';
import { listBacklog, syncAcceptedDecisions, decideBacklogItem } from './decision-backlog.mjs';
import { creativeDecay } from './creative-decay.mjs';
import { buildCampaignPostmortem, saveCampaignPostmortem, listCampaignPostmortems } from './campaign-postmortem.mjs';
...
app.get('/api/creative-memory',(req,res)=>{const projects=list(),perf=performanceSummary(DATA,projects),experiments=experimentInsights(DATA);refreshCreativeMemory(DATA,projects,perf,experiments);res.json(retrieveCreativeMemory(DATA,{niche:String(req.query.niche||''),style:String(req.query.style||''),platform:String(req.query.platform||''),voice:String(req.query.voice||''),hook:String(req.query.hook||''),limit:Number(req.query.limit||8)}))});
...
function campaignLoopSnapshot(campaign){const projects=list(),perf=performanceSummary(DATA,projects),learning=crossPlatformLearning(listPlatformPerformance(DATA),projects),creative=creativeIntelligence(projects,perf,experimentInsights(DATA));return {projects,perf,learning,creative,plan:campaignLoopPlan(campaign,{projects,performance:perf,publishJobs:listPublishJobs(DATA,{}),creative,learning})};}
app.get('/api/campaigns/:id/loop',(req,res)=>{const c=getCampaign(DATA,req.params.id);if(!c)return res.status(404).json({error:'campaign not found'});res.json(campaignLoopSnapshot(c).plan)});
app.post('/api/campaigns/:id/auto-schedule',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});if(req.body?.confirm!==true)return res.status(409).json({error:'explicit confirm=true is required'});const c=getCampaign(DATA,req.params.id);if(!c)return res.status(404).json({error:'campaign not found'});try{const snap=campaignLoopSnapshot(c),ids=new Set(c.projectIds||[]),existing=listPublishJobs(DATA,{}),projects=snap.projects.filter(p=>ids.has(p.id)&&p.status==='complete'&&p.qa?.launchReady===true&&p.publishApproval?.status==='approved'),calendar=publishingCalendar(DATA,{days:Number(req.body?.days||30)}),schedule=campaignSchedule(c,calendar,{slots:Math.min(projects.length,Number(req.body?.slots||projects.length||1))}),platforms=(c.channels||[]).filter(x=>['youtube-shorts','tiktok','instagram-reels'].includes(x)),created=[];for(let i=0;i<projects.length&&i<schedule.suggestions.length;i++){const project=projects[i],platform=platforms[i%Math.max(1,platforms.length)]||'youtube-shorts';if(existing.some(j=>j.projectId===project.id&&j.platform===platform&&['ready','scheduled','published','publishing'].includes(j.status)))continue;created.push(createPublishJob(DATA,project,{platform,scheduledAt:schedule.suggestions[i].scheduledAt}));}res.status(201).json({campaignId:c.id,created,plan:snap.plan,policy:{ownerConfirmed:true,autoPublish:false,approvalRequired:true}})}catch(e){res.status(409).json({error:String(e.message||e)})}});
app.get('/api/channel-goals',(req,res)=>{const perf=performanceSummary(DATA,list()),jobs=listPublishJobs(DATA,{});res.json({goals:listChannelGoals(DATA).map(g=>({...g,progress:channelGoalProgress(g,perf,jobs)}))})});
app.post('/api/channel-goals',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});try{res.status(201).json(createChannelGoal(DATA,req.body||{}))}catch(e){res.status(400).json({error:String(e.message||e)})}});
app.patch('/api/channel-goals/:id',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});try{res.json(updateChannelGoal(DATA,req.params.id,req.body||{}))}catch(e){res.status(400).json({error:String(e.message||e)})}});
app.get('/api/system-loop/backlog',(req,res)=>res.json({items:listBacklog(DATA,{status:String(req.query.status||'')}),policy:{ownerReviewRequired:true,autoExecute:false}}));
app.patch('/api/system-loop/backlog/:id',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});try{res.json(decideBacklogItem(DATA,req.params.id,String(req.body?.status||'')))}catch(e){res.status(400).json({error:String(e.message||e)})}});
app.get('/api/creative-decay',(req,res)=>{const projects=list(),perf=performanceSummary(DATA,projects),memory=refreshCreativeMemory(DATA,projects,perf,experimentInsights(DATA));res.json(creativeDecay(memory))});
app.get('/api/campaigns/:id/postmortem',(req,res)=>{const c=getCampaign(DATA,req.params.id);if(!c)return res.status(404).json({error:'campaign not found'});const projects=list(),perf=performanceSummary(DATA,projects),kpi=campaignKpiProgress(c,perf,listPublishJobs(DATA,{})),learning=listCampaignLearningReports(DATA).find(x=>x.campaignId===c.id)||null,memory=refreshCreativeMemory(DATA,projects,perf,experimentInsights(DATA)),report=saveCampaignPostmortem(DATA,buildCampaignPostmortem(c,{kpiProgress:kpi,learningReport:learning,decay:creativeDecay(memory)}));res.json(report)});
app.get('/api/campaigns/postmortems',(req,res)=>res.json({reports:listCampaignPostmortems(DATA)}));
app.get('/api/system-loop/evaluation',(req,res)=>res.json(recommendationEvaluation(listLoopCycles(DATA,{limit:100}),decisionFeedbackSummary(DATA),performanceTrend(DATA,{days:Number(req.query.days||90)}).points||[])));
app.get('/api/system-loop/plan',(req,res)=>{const projects=list(),perf=performanceSummary(DATA,projects),learning=crossPlatformLearning(listPlatformPerformance(DATA),projects),creative=creativeIntelligence(projects,perf,experimentInsights(DATA)),campaigns=listCampaigns(DATA),plans=campaigns.filter(c=>['planning','active'].includes(c.status)).map(c=>campaignLoopPlan(c,{projects,performance:perf,publishJobs:listPublishJobs(DATA,{}),creative,learning})),runs=listRuns(DATA).map(run=>syncRunWithProjects(DATA,run,load));res.json(nextBestActions({campaigns,campaignPlans:plans,performance:perf,creative,learning,runs,feedback:decisionFeedbackSummary(DATA)}))});
app.get('/api/system-loop/history',(req,res)=>res.json({cycles:listLoopCycles(DATA,{limit:Number(req.query.limit||30)}),feedback:decisionFeedbackSummary(DATA)}));
app.get('/api/system-loop/posting-times',(req,res)=>res.json(postingTimeIntelligence(listPlatformPerformance(DATA),listPublishJobs(DATA,{}))));
app.post('/api/system-loop/feedback',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});try{res.status(201).json(recordDecisionFeedback(DATA,req.body||{}))}catch(e){res.status(400).json({error:String(e.message||e)})}});
app.get('/api/system-loop/recovery-candidates',(req,res)=>res.json({candidates:recoveryCandidates(list(),listRuns(DATA).map(run=>syncRunWithProjects(DATA,run,load))),policy:{ownerConfirmationRequired:true,autoPublish:false}}));
app.post('/api/system-loop/recover',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});if(req.body?.confirm!==true)return res.status(409).json({error:'explicit confirm=true is required'});const projects=list(),runs=listRuns(DATA).map(run=>syncRunWithProjects(DATA,run,load)),candidates=recoveryCandidates(projects,runs),recovered=[],skipped=[];for(const c of candidates.slice(0,Math.max(1,Math.min(10,Number(req.body?.limit||5))))){if(c.type==='project'){const j=load(c.id);if(!j||j.status!=='failed'||classifyRecoverability(j).recoverable===false){skipped.push(c);continue}prepareProjectRetry(j);save(j);setImmediate(kick);recovered.push({type:'project',id:j.id});continue}const run=getRun(DATA,c.id),projectId=c.projectId;if(!run||!projectId){skipped.push(c);continue}const j=load(projectId);if(!j||j.status!=='failed'||classifyRecoverability(j).recoverable===false){skipped.push(c);continue}prepareProjectRetry(j);save(j);setImmediate(kick);updateRun(DATA,run.id,{status:'producing',checkpoint:{stage:'video-production',projectId:j.id,progress:Number(j.progress||0)},error:null});recovered.push({type:'run',id:run.id,projectId:j.id});}res.status(202).json({recovered,skipped,policy:{ownerConfirmed:true,autoPublish:false,freeOnly:true}})});
app.post('/api/system-loop/run',(req,res)=>{if(req.authActor?.type!=='owner')return res.status(403).json({error:'owner required'});if(req.body?.confirm!==true)return res.status(409).json({error:'explicit confirm=true is required'});const projects=list(),perf=performanceSummary(DATA,projects),learning=crossPlatformLearning(listPlatformPerformance(DATA),projects),creative=creativeIntelligence(projects,perf,experimentInsights(DATA));refreshCreativeMemory(DATA,projects,perf,experimentInsights(DATA));const campaigns=listCampaigns(DATA).filter(c=>['planning','active'].includes(c.status)),plans=campaigns.map(c=>campaignLoopPlan(c,{projects,performance:perf,publishJobs:listPublishJobs(DATA,{}),creative,learning})),reports=campaigns.filter(c=>(c.projectIds||[]).length).map(c=>saveCampaignLearningReport(DATA,buildCampaignLearningReport(c,{projects,performance:perf,publishJobs:listPublishJobs(DATA,{}),creative,learning}))),runs=listRuns(DATA).map(run=>syncRunWithProjects(DATA,run,load)).map(run=>({...run,handoffs:syncAgentHandoffs(DATA,run)})),feedback=decisionFeedbackSummary(DATA),decisions=nextBestActions({campaigns,campaignPlans:plans,performance:perf,creative,learning,runs,feedback}),backlog=syncAcceptedDecisions(DATA,decisions.actions,feedback),cycle=recordLoopCycle(DATA,{actions:decisions.actions,campaignReports:reports.length,runsChecked:runs.length,ownerConfirmed:true}),evaluation=recommendationEvaluation(listLoopCycles(DATA,{limit:100}),feedback,performanceTrend(DATA,{days:90}).points||[]);res.json({status:'cycle-complete',cycle,decisions,backlog:backlog.slice(-20),evaluation,campaignReports:reports.length,runsChecked:runs.length,postingTimes:postingTimeIntelligence(listPlatformPerformance(DATA),listPublishJobs(DATA,{})),feedback,policy:{ownerConfirmed:true,autoPublish:false,freeOnly:true}})});
