...
async function diagnosticsSnapshot(allProjects=list(),providerSnapshot=providerJobsSnapshot(DATA)){
  ...
  return {status:critical.every(x=>x.ok)?'ready':'degraded',checks,storage:{freeBytes,totalBytes,freePercent},queue:{pending,recentProviderFailures,historicalProviderFailures:providerFailures},failureSummary:{recentProjects:recentProjectFailures.length,historicalProjects:failedProjects.length},approvalSummary,recentProjectFailures,activeProject:active,shuttingDown,generatedAt:new Date().toISOString()};
}
...
const maintainProviderState=()=>maintainProviderJobs(DATA,{projectIds:list().map(x=>x.id),retentionDays:Number(process.env.PROVIDER_JOB_RETENTION_DAYS||30)});
