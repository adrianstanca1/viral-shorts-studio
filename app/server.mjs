...
import { googleSetupTokenValid, saveGoogleOAuthClient, googleSetupPage, googleSetupSuccessPage } from './google-setup.mjs';
...
app.get('/login',(req,res)=>{if(!authConfigured(AUTH_SECRET))return res.redirect('/');const gs=googleOAuthStatus(DATA);res.type('html').send(loginPage(req.query.error==='1',{googleEnabled:gs.loginEnabled,googleError:req.query.google_error==='1'}));});
...
app.get('/setup/google',(req,res)=>{res.setHeader('Cache-Control','no-store');const token=String(req.query.token||'');if(!googleSetupTokenValid(DATA,token))return res.status(410).type('html').send('<!doctype html><title>Setup link expired</title><p>This Google setup link is invalid or expired.</p>');return res.type('html').send(googleSetupPage(token));});
app.post('/setup/google',createRateLimiter({windowMs:15*60_000,max:8}),(req,res)=>{res.setHeader('Cache-Control','no-store');const token=String(req.body?.token||'');try{saveGoogleOAuthClient(DATA,{token,clientId:req.body?.clientId,clientSecret:req.body?.clientSecret,ownerEmail:req.body?.ownerEmail});return res.type('html').send(googleSetupSuccessPage())}catch(e){const msg=String(e.message||e).slice(0,180);if(!googleSetupTokenValid(DATA,token))return res.status(410).type('html').send('<!doctype html><title>Setup link expired</title><p>This Google setup link is invalid or expired.</p>');return res.status(400).type('html').send(googleSetupPage(token,{error:msg}))}});
