...
import { listWorkspaceMembers, createWorkspaceInvite, acceptWorkspaceInvite, revokeWorkspaceMember, verifyWorkspaceMember, memberCanAccess } from './workspace-members.mjs';
...
app.post('/api/workspace/invites/accept',createRateLimiter({windowMs:15*60_000,max:10}),(req,res)=>{try{res.json(acceptWorkspaceInvite(DATA,req.body?.inviteToken))}catch(e){res.status(400).json({error:String(e.message||e)})}});
app.use((req,res,next)=>{if(req.path==='/api/health'||req.path==='/login'||req.path==='/api/session'||req.path==='/recover-owner'||req.path==='/recover-owner/request'||req.path==='/recover-owner/complete'||req.path==='/api/workspace/invites/accept')return next();if(WORKER_TOKEN&&safeEqual(req.get('x-provider-worker-token'),WORKER_TOKEN)){req.authActor={type:'worker',id:'provider-worker',role:'worker'};return next();}if(isOwner(req,AUTH_SECRET)){req.authActor={type:'owner',id:'owner',role:'owner'};return next();}const raw=String(req.get('authorization')||'').replace(/^Bearer\s+/i,'').trim(),member=verifyWorkspaceMember(DATA,raw);if(member){if(!memberCanAccess(member,req.method,req.path))return res.status(403).json({error:'workspace role does not permit this action'});req.workspaceMember=member;req.authActor={type:'member',id:member.id,role:member.role};return next();}if(req.path.startsWith('/api/'))return res.status(401).json({error:'authentication required'});return res.redirect('/login');});
...
app.get('/api/voices/:id/preview',(req,res)=>{const voice=getVoice(req.params.id);if(!voice)return res.status(404).json({error:'voice not found'});const dir=path.join(DATA,'voice-previews');fs.mkdirSync(dir,{recursive:true});const text=voicePreviewText(req.query.text),key=crypto.createHash('sha256').update(`${voice.id}|${text}`).digest('hex').slice(0,16),out=path.join(dir,`${voice.id}-${key}.wav`);if(!fs.existsSync(out)){const r=spawnSync('ffmpeg',['-y','-f','lavfi','-i',`flite=text='${text.replace(/'/g,' ')}':voice=${voice.id}`,'-ar','44100','-ac','1',out],{stdio:'ignore',timeout:20000});if(r.status!==0||!fs.existsSync(out))return res.status(500).json({error:'voice preview generation failed'});}res.type('audio/wav').sendFile(out);});
