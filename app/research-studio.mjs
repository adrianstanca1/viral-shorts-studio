...
export async function runResearch(root,input={}){const query=String(input.query||'').trim().slice(0,300);if(query.length<3)throw new Error('research query is required');const mode=['topic','trend','competitor','faceless'].includes(input.mode)?input.mode:'topic';const sources=await researchTopic(query);const report={id:crypto.randomUUID(),query,mode,createdAt:new Date().toISOString(),sourceCount:sources.length,sources:sources.slice(0,8),opportunities:opportunities(query,sources),disclaimer:mode==='trend'?'Opportunity scores are evidence signals, not platform search-volume measurements.':mode==='faceless'?'Channel/revenue estimates are not claimed without channel analytics data.':'Source-grounded research summary.'};const x=read(root);x.reports=[report,...(x.reports||[])].slice(0,100);write(root,x);return report}
