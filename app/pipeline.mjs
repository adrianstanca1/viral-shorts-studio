...
export async function makeScene(scene,dir,fallbackQuery,mediaPool=[],videoPool=[]){
...
export async function produceProject(project,root,onUpdate=()=>{}){
  ...
  try{
    ...
    const renderOne=async(scene)=>{
      ...
      let result,last;
      for(let attempt=0;attempt<2;attempt++){try{result=await makeScene(scene,dir,project.topic,mediaPool,videoPool);break;}catch(e){last=e;}}
      if(!result)throw last; return result;
    };
