...
export function editingRhythmAnalysis(scenes=[],storyboard=[]){
  ...
  for(let i=0;i<scenes.length;i++){const scene=scenes[i]||{},keys=repeatableKeys(scene);
    ...
    const mode=scene.visualType||'unknown';const firstVisual=[...keys][0]||'';const signature=mode==='archive-motion'?`${mode}:${firstVisual}`:mode==='whiteboard'?`${mode}:${scene.index}`:mode;run=signature===lastType?run+1:1;lastType=signature;longestRun=Math.max(longestRun,run);if(run>=4)runRepair.push(scene.index);
  }
