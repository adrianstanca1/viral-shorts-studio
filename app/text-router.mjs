...
async function huggingFaceCloud(req){
  ...
  const model=process.env.HF_TEXT_MODEL||'openai/gpt-oss-120b';
...
export async function completeText(req){
  ...
  for(const id of order){try{
    ...
    if(id==='ollama-cloud')return await ollamaCloud(req);
    if(id==='huggingface')return await huggingFaceCloud(req);
...
export async function textProviderStatus(){
  const out={localFirst:true,paidFallback:false,providers:[]};
  ...
  out.providers.push({id:'ollama-cloud',enabled:truthy(process.env.OLLAMA_CLOUD_ENABLED)&&!!process.env.OLLAMA_API_KEY,status:truthy(process.env.OLLAMA_CLOUD_ENABLED)?'configured':'disabled',model:process.env.OLLAMA_CLOUD_MODEL||'gpt-oss:120b-cloud',policy:'included-plan/cloud fallback'});
  const hf=hfAllowance();out.providers.push({id:'huggingface-cloud',enabled:truthy(process.env.HF_CLOUD_ENABLED)&&!!process.env.HF_TOKEN,status:truthy(process.env.HF_CLOUD_ENABLED)?'guarded-free-tier':'disabled',model:process.env.HF_TEXT_MODEL||'openai/gpt-oss-120b',dailyCallGuard:hf.limit,remainingToday:hf.remaining});
