...
import { textRoutingPlan, learnedProviderScore, rankedCloudModels } from './text-router.mjs';
...
assert.ok(learnedProviderScore({attempts:10,successes:10,latencyMs:1000})>learnedProviderScore({attempts:10,successes:3,latencyMs:60000}));const modelRanks=rankedCloudModels('huggingface',{task:'fact-check',quality:'strong'});assert.ok(modelRanks.length>=4);assert.ok(modelRanks[0].rankScore>=modelRanks.at(-1).rankScore);
