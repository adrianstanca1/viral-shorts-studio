...
const creatorRoot=fs.mkdtempSync(path.join(os.tmpdir(),'viral-creator-assets-test-'));assert.ok(imageKinds().includes('youtube-thumbnail'));const imageBrief=createImageBrief(creatorRoot,{kind:'youtube-thumbnail',prompt:'Roman history mystery'});assert.equal(imageBrief.aspect,'16:9');const character=createCharacter(creatorRoot,{name:'Marcus',description:'Roman historian'});assert.equal(getCharacter(creatorRoot,character.id).name,'Marcus');fs.rmSync(creatorRoot,{recursive:true,force:true});assert.equal(captionWordsForStyle('bold'),4);assert.equal(captionWordsForStyle('documentary'),5);assert.equal(captionWordsForStyle('minimal'),7);assert.equal(sceneCountForDuration(30),8);assert.equal(sceneCountForDuration(300,'single-scene'),1);assert.equal(sceneCountForDuration(60),14);assert.equal(sceneCountForDuration(90),20);assert.equal(sceneCountForDuration(300),40);assert.equal(sceneCountForDuration(1200),160);
