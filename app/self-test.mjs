...
import { createGoogleSetupToken, googleSetupTokenValid, saveGoogleOAuthClient } from './google-setup.mjs';
...
const setup=createGoogleSetupToken(oauthRoot,{ttlMinutes:5});assert.equal(googleSetupTokenValid(oauthRoot,setup.token),true);saveGoogleOAuthClient(oauthRoot,{token:setup.token,clientId:'stored-client-id-1234567890',clientSecret:'stored-secret-123',ownerEmail:'stored@example.com'});assert.equal(googleSetupTokenValid(oauthRoot,setup.token),false);const storedStatus=googleOAuthStatus(oauthRoot,{env:{}});assert.equal(storedStatus.configured,true);assert.equal(storedStatus.loginEnabled,true);assert.equal(googleLoginAuthorized({sub:'stored',email:'stored@example.com',email_verified:true},oauthRoot,{env:{}}),true);assert.ok(beginGoogleOAuth({flow:'login',redirectUri:'https://example.com/api/oauth/google/callback',root:oauthRoot,env:{}}).includes('stored-client-id-1234567890'));
