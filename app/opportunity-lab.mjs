...
export function buildOpportunityScorecard(query,reports=[]){
  ...
  const hasStore=modes.includes('store'),hasAds=modes.includes('ad');return {id:crypto.randomUUID(),query:String(query||'').slice(0,300),createdAt:new Date().toISOString(),modes,sourceCount,opportunities,connectors:{facelessFinder:'evidence-derived',trendFinder:'evidence-derived',storeFinder:hasStore?'web-evidence':'connector-required',metaAdLibrary:hasAds?'web-evidence':'connector-required'},policy:{sourceGrounded:true,platformVolumeClaimed:false,revenueClaimed:false,nativePlatformDataset:false,autoAct:false}};
}
