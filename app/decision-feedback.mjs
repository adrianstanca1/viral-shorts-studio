...
export function decisionFeedbackSummary(root){const rows=read(root).items||[],byType={};for(const r of rows){const x=byType[r.decisionType]||{accepted:0,rejected:0};x[r.decision]=(x[r.decision]||0)+1;byType[r.decisionType]=x}return {total:rows.length,byType,policy:{advisoryOnly:true,autoExecute:false}}}
