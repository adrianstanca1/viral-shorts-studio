...
function clientId(env=process.env){return env.GOOGLE_CLIENT_ID||env.YOUTUBE_CLIENT_ID||'';}
function clientSecret(env=process.env){return env.GOOGLE_CLIENT_SECRET||env.YOUTUBE_CLIENT_SECRET||'';}
...
function allowedEmails(env=process.env){return String(env.GOOGLE_OWNER_EMAILS||env.GOOGLE_OWNER_EMAIL||'').split(',').map(cleanEmail).filter(Boolean);}
...
export function googleOAuthStatus(root,{env=process.env}={}){
  const store=readGoogleOAuthStore(root),configured=!!(clientId(env)&&clientSecret(env));
  const allow=allowedEmails(env),paired=!!store.owner?.sub;
...
export function googleLoginAuthorized(profile,root,{env=process.env}={}){
  if(!profile?.email_verified)return false;const email=cleanEmail(profile.email),store=readGoogleOAuthStore(root),allow=allowedEmails(env);
...
export function youtubeCredentialEnv(root,{env=process.env}={}){
  const store=readGoogleOAuthStore(root);return {...env,YOUTUBE_CLIENT_ID:env.YOUTUBE_CLIENT_ID||env.GOOGLE_CLIENT_ID||'',YOUTUBE_CLIENT_SECRET:env.YOUTUBE_CLIENT_SECRET||env.GOOGLE_CLIENT_SECRET||'',YOUTUBE_REFRESH_TOKEN:env.YOUTUBE_REFRESH_TOKEN||store.youtube?.refreshToken||''};
}
export function beginGoogleOAuth({flow,redirectUri,env=process.env}){
  if(!clientId(env)||!clientSecret(env))throw new Error('Google OAuth client is not configured');
  purgeStates();const state=b64url(crypto.randomBytes(24)),verifier=b64url(crypto.randomBytes(48));states.set(state,{flow,verifier,redirectUri,expiresAt:Date.now()+10*60_000});
  const scopes=flow==='youtube'?['openid','email','profile',YOUTUBE_SCOPE]:['openid','email','profile'];
  const q=new URLSearchParams({client_id:clientId(env),redirect_uri:redirectUri,response_type:'code',scope:scopes.join(' '),state,code_challenge:challenge(verifier),code_challenge_method:'S256',include_granted_scopes:'true'});
  if(flow==='youtube'){q.set('access_type','offline');q.set('prompt','consent');}
  return `${AUTH_URL}?${q}`;
}
export async function finishGoogleOAuth({code,state,env=process.env}){
  ...
  const body=new URLSearchParams({code:String(code||''),client_id:clientId(env),client_secret:clientSecret(env),redirect_uri:pending.redirectUri,grant_type:'authorization_code',code_verifier:pending.verifier});
