...
The text router is task-aware and adaptive. Fast/balanced work stays local-first on Qwen3, while strong fact-check, research, reasoning, and high-quality storyboard requests prefer only currently enabled free/included cloud routes before falling back to local Ollama. Disabled providers are skipped instead of adding latency. The manager records per-provider attempts, successes, failures, smoothed latency, cooldowns, and the exact routing plan in provider status. Cloud providers use circuit breakers on authentication/quota failures; no paid fallback is allowed.
